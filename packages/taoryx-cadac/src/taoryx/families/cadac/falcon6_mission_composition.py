...
def _output(
    ...
    *,
    ...
    return TrajectoryOutputChannelMetadata(
        ...
        shape=shape,
        frame=frame,
        interpolation="step" if data_type == "boolean" else "linear",
...
def _build_output_schema() -> TrajectoryOutputSchema:
    ...
    telemetry = (
        ...
        _output(
            ...
            "Surface position limit",
            "Per-surface source actuator position-limit flags in aileron/elevator/rudder order.",
            shape=(3,),
            data_type="boolean",
        ),
        _output(
            ...
            "Surface rate limit",
            "Per-surface source actuator rate-limit flags in aileron/elevator/rudder order.",
            shape=(3,),
            data_type="boolean",
        ),
...
def _sample_value(channel_id: str, sample: Falcon6PlantSample) -> object:
    values: dict[str, object] = {
        ...
        "surface_position_limited": sample.surface_position_limited,
        "surface_rate_limited": sample.surface_rate_limited,
