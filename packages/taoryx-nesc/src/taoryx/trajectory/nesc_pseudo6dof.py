...
def _scaled_profile(profile: Pseudo6DOFProfile, response_time_scale: float) -> Pseudo6DOFProfile:
    ...
    if response_time_scale == 1.0:
        return profile
    response = {name: axis.model_copy(update={"time_constant_s": axis.time_constant_s * response_time_scale}) for name, axis in profile.response.items()}
    phase_response = {
        phase: {name: axis.model_copy(update={"time_constant_s": axis.time_constant_s * response_time_scale}) for name, axis in axes.items()}
