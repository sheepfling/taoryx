...
def build_f16_point_tuning_campaign() -> TuningCampaign:
    """Return the source-trim F-16 translational local-control screen.

    This regulates the retained body-velocity coordinates of the point-mass
    reduction.  It deliberately remains LQR: this realization carries no
    position or attitude output whose offset-free tracking would be a
    meaningful claim.
    """

    return ControlAutomationDeclaration(
        id="f16-point-source-trim-translation",
        campaign_id="f16-point-source-trim-translation-v1",
        family_id="f16_s119",
        tier="point_mass_3dof",
        strategy_id="powered_fixed_wing.v1",
        node_id="source-trim-translation",
        state_scales={
            "u_m_s": 100.0,
            "v_m_s": 10.0,
            "w_m_s": 10.0,
        ...
        authority_state_names=F16_POINT_STATE_NAMES,
    ).build_campaign()
    ####


def build_f16_pseudo_tuning_campaign() -> TuningCampaign:
    """Return the source-trim F-16 attitude-response LQI screen.

    The pseudo-6DOF model retains kinematic attitude coordinates, so the
    shared LQI path integrates the three declared attitude errors.  It still
    does not represent a physical F-16 surface-allocation controller.
    """

    return ControlAutomationDeclaration(
        id="f16-pseudo-source-trim-attitude",
        campaign_id="f16-pseudo-source-trim-attitude-v1",
        family_id="f16_s119",
        tier="pseudo_6dof",
        strategy_id="powered_fixed_wing.v1",
        node_id="source-trim-attitude-response",
        state_scales={
            "u_m_s": 100.0,
            "v_m_s": 10.0,
            "w_m_s": 10.0,
        ...
        authority_state_names=F16_PSEUDO_STATE_NAMES,
