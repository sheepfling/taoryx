...
class SimpleAeroTrajectoryBuilder:
    ...
    def derive(self) -> DerivedFixedLDProfile:
        ...
        return DerivedFixedLDProfile(
            ...
            assumptions=(
                ...
                "family is provenance metadata; the builder emits a declared fixed-L/D segment graph",
            ),
        )
        ####
    ...
    def _segment_lines(self, derived: DerivedFixedLDProfile) -> tuple[str, ...]:
        ...
        for index, item in enumerate(raw_segments, start=1):
            ...
            if source_segment_id is not None:
                lines.append(
                    f"    # source Simple Aero segment {source_segment_id}; lowered to fixed-L/D {kind}"
                )
            ...
            elif kind in {"ballistic_coast", "alpha_profile", "skip_maneuver"}:
                alpha_deg = float(item.get("alpha_deg", p.alpha_deg))
                if kind == "alpha_profile" and "alpha_profile_deg" in item:
                    profile = item["alpha_profile_deg"]
                    if not isinstance(profile, (list, tuple)) or len(profile) < 2:
                        raise ValueError("an alpha-profile segment requires at least two [time, alpha] points")
                    lines.append("    *fly alpha vrs tseg")
                    for point in profile:
                        if not isinstance(point, (list, tuple)) or len(point) != 2:
                            raise ValueError("alpha-profile points must be [time_s, alpha_deg] pairs")
                        time_s, profile_alpha_deg = (float(value) for value in point)
                        if not math.isfinite(time_s) or not math.isfinite(profile_alpha_deg) or time_s < 0.0:
                            raise ValueError("alpha-profile values must be finite and use nonnegative times")
                        lines.append(f"      {time_s:g} {profile_alpha_deg:g}")
                else:
                    lines.append(f"    *fly alpha={alpha_deg:g}")
                if "bank_deg" in item:
                    bank_deg = float(item["bank_deg"])
                    if not math.isfinite(bank_deg):
                        raise ValueError("alpha-profile bank command must be finite")
                    lines.append(f"    *fly bankgd={bank_deg:g}")
                lines.append(f"    *when tseg={duration:g} {target}")
            elif kind == "bank_maneuver":
                bank_deg = float(item.get("bank_deg", p.bank_deg))
                if not math.isfinite(bank_deg):
                    raise ValueError("bank-maneuver command must be finite")
                lines.append(f"    *fly bankgd={bank_deg:g}")
                lines.append(f"    *when tseg={duration:g} {target}")
    ...
    def build(self) -> SimpleAeroTrajectoryBuild:
        ...
        d = d.model_copy(update={"total_duration_s": total})
