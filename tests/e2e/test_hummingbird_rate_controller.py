...
RATE_LQR_PROBLEM = ROOT / "examples/mission_families/slower_hummingbird/SV05_rate_lqr_hover_6dof.prb"
...
@pytest.mark.dof6
@pytest.mark.hummingbird
def test_hummingbird_rate_lqr_uses_realization_and_individual_rotor_commands(tmp_path: Path) -> None:
    """The rate LQR witness uses the common contract without changing the baseline path."""

    report = run_files(RATE_LQR_PROBLEM, TABLES, output_dir=tmp_path, max_steps=1000, profile=GrammarProfile.TAORYX)
    assert report.exit_code == 0, [(item.code, item.message) for item in report.diagnostics]
    assert report.results and report.results[0].completed
    assert report.metadata
    realization = report.metadata[0]["controller_realization"]
    assert isinstance(realization, dict)
    assert realization["role"] == "rate"
    assert realization["implementation"] == "lqr"
    assert realization["implementation_version"] == "runtime-scaled-lqr-v1"
    assert realization["control_path"] == ["guidance", "rate_lqr", "allocator", "actuator", "plant"]
    history = report.results[0].states["1"]
    initial_rate = sum(history[0].named[name] ** 2 for name in ("wx", "wy", "wz")) ** 0.5
    final_rate = sum(history[-1].named[name] ** 2 for name in ("wx", "wy", "wz")) ** 0.5
    assert final_rate < initial_rate
    assert max(abs(state.named["rate_lqr_request_moment_x_nm"]) for state in history) > 0.0
    assert max(abs(state.named["rate_lqr_request_moment_y_nm"]) for state in history) > 0.0
    assert max(abs(state.named["rate_lqr_request_moment_z_nm"]) for state in history) > 0.0
    assert all(0.0 <= state.named[f"aero_query_rotor-{index}-speed"] <= 1500.0 for state in history for index in range(1, 5))
    ####
