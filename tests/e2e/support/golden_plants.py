...
class GoldenPlantRun:
    ...
    def require_finite(self) -> None:
        ...
        for sample in self.history:
            angle_names = {"alpha", "beta", "aero_alpha_deg", "aero_sideslip_deg"}
            for name, value in sample.items():
                if name in {"aero_table_min_margin", "aero_table_min_normalized_margin"} and math.isnan(value):
                    continue
                if name in angle_names and sample.get("aero_air_data_valid", 1.0) < 0.5:
                    assert math.isnan(value)
                    continue
                if name in angle_names or name in {"aero_table_min_margin", "aero_table_min_normalized_margin"}:
