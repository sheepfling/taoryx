...
def test_product_two_catalog_declares_isolated_lanes_and_refinement_cases() -> None:
    catalog = load_product_two_catalog()

    assert validate_product_two_quality_catalog(catalog.scenarios) == ()
...
def test_product_two_quality_passes_repeatability_refinement_and_declared_mass_balance() -> None:
    source = load_product_two_catalog().find("two-stage-ballistic")
    ...
    report = evaluate_product_two_quality(scenario, primary=first, repeat=second, coarse=coarse, fine=fine)
...
def test_product_two_quality_rejects_nonfinite_declared_telemetry_and_identity_drift() -> None:
    source = load_product_two_catalog().find("two-stage-ballistic")
    quality = source.quality.model_copy(
        update={
    ...
    report = evaluate_product_two_quality(scenario, primary=first, repeat=second)
