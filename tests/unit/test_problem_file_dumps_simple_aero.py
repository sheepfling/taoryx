...
from taoryx.fixtures.simple_aero_dumps import (
...
ROOT = Path(__file__).resolve().parents[1] / "fixtures" / "problem_file_dumps" / "simple_aero_segments"
SPEC = ROOT / "spec.yaml"
CATALOG = ROOT / "catalog.yaml"
pytestmark = pytest.mark.simple_aero


def test_simple_aero_segment_spec_renders_the_checked_in_problem_files() -> None:
    spec = load_spec(SPEC)
...
def test_simple_aero_segment_manifest_matches_problem_files() -> None:
    ...
    assert manifest["leaf"] == "simple_aero_segments"
    assert manifest["source_bundle"] == "../../simple_aero_v1"
    assert manifest["generated_catalog"] == "catalog.yaml"
####


def test_simple_aero_segment_catalog_groups_families_by_phase() -> None:
...
def test_simple_aero_segment_problem_files_are_minimally_structured() -> None:
