...
def test_spectre_contracts_cover_every_family_and_preserve_reuse_boundaries() -> None:
    catalog = yaml.safe_load((ROOT / "verification/spectre_segment_catalog.yaml").read_text(encoding="utf-8"))
    template_names = {item["name"] for item in catalog["templates"]}
    family_names = {item["name"] for item in catalog["families"]}

    assert family_names == set(SPECTRE_FAMILY_SEGMENTS)
    assert {contract.name for contract in SPECTRE_SEGMENT_CONTRACTS} >= template_names | {"moving_target_intercept"}
    ...
    for family in family_names:
        contracts = spectre_family_segments(family)
...
def test_spectre_fixture_paths_and_sources_are_present() -> None:
    catalog = yaml.safe_load((ROOT / "verification/spectre_segment_catalog.yaml").read_text(encoding="utf-8"))
    for family in catalog["families"]:
        assert (ROOT / "tests/fixtures/problem_file_dumps/spectre_trajectories" / family["fixture"]).is_file()
        if family["source"].endswith(".yaml"):
            assert (ROOT / "tests/fixtures/spectre_simple_aero_v1" / family["source"]).is_file()
        else:
