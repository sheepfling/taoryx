"""Build the Product 2 M4 numerical-quality report."""
...
from taoryx.product_two_catalog import ROOT, ProductTwoScenario, load_product_two_catalog
from taoryx.product_two_quality import (
...
DEFAULT_OUTPUT = ROOT / "artifacts/verification/product_two_quality/report.json"


def build_product_two_quality_suite(
    *,
    ...
    catalog = load_product_two_catalog(catalog_path or ROOT / "verification/product_two_scenario_catalog.yaml")
    catalog_errors = validate_product_two_quality_catalog(catalog.scenarios)
    if catalog_errors:
        raise ValueError("invalid Product 2 M4 catalog: " + "; ".join(catalog_errors))
    selected = set(selected_scenarios)
    executed: list[str] = []
    reports: list[ProductTwoNumericalQualityReport] = []
    with tempfile.TemporaryDirectory(prefix="taoryx-product-two-quality-") as temporary:
        temporary_root = Path(temporary)
        for scenario in catalog.scenarios:
            if not execute or (selected and scenario.id not in selected):
                reports.append(evaluate_product_two_quality(scenario))
                continue
            executed.append(scenario.id)
            try:
                primary, repeat, coarse, fine = _execute_scenario(scenario, temporary_root / scenario.id)
            except (OSError, RuntimeError, TypeError, ValueError) as error:
                report = evaluate_product_two_quality(scenario)
    ...
    return {
        "schema": "taoryx.product-two-quality-suite/v1alpha1",
        "schema_version": 1,
...
def _execute_scenario(
    ...
    if scenario.quality.artifact_required or scenario.entrypoint in {"interactive", "composition"}:
        build_product_two_bundle(scenario, root / "primary", root=ROOT, run=True, plots=False)
        build_product_two_bundle(scenario, root / "repeat", root=ROOT, run=True, plots=False)
...
def _run_source(scenario: ProductTwoScenario, output: Path, *, multiplier: float, integrator: str) -> Any:
...
def _append_execution_failure(report: ProductTwoNumericalQualityReport, message: str) -> ProductTwoNumericalQualityReport:
    check = ProductTwoQualityCheck(
        id="execution",
...
def _suite_disposition(reports: Sequence[ProductTwoNumericalQualityReport]) -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--catalog", type=Path, default=ROOT / "verification/product_two_scenario_catalog.yaml")
    ...
    report = build_product_two_quality_suite(
