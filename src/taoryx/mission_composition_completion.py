...
from .trajectory.contract_probe_mission_composition import ContractProbeMissionCompositionProvider
...
from .trajectory.reference_mission_composition import ReferenceMissionCompositionProvider
...
def build_mission_composition_completion_report(
    ...
    unknown_backlog_families = sorted(
        {
            family
            for item in selected_backlog.items
            for family in item.families
            if family not in model_ids
        }
    )
    diagnostics = [*inventory_audit.errors]
    ...
    families = tuple(
        _family_coverage(model, selected_inventory)
        for model in selected_provider.list_models()
    )
    registered_batch = sum(
        len(row.batch_tuples)
        for family in families
        for row in family.realizations
        if row.batch_status == "available"
    )
    registered_interactive = sum(
        len(row.interactive_tuples)
        for family in families
        for row in family.realizations
        if row.interactive_status == "available"
    )
    return MissionCompositionCompletionReport(
        ...
        completion_rule=(
            ...
            "all other combinations are explicitly blocked or unsupported."
        ),
...
def _family_coverage(
    ...
    return MissionCompositionFamilyCoverage(
        ...
        realizations=tuple(
            _realization_coverage(model, realization, inventory_status, inventory)
...
def _realization_coverage(
    ...
    available_operations = {
        operation
        for operation, status in (("batch", batch_status), ("step", interactive_status))
        if status == "available"
    }
    telemetry = tuple(
        sorted(
            ...
            and (
                not channel.compatible_realizations
                or realization.id in channel.compatible_realizations
            )
            and (
                not channel.compatible_fidelities
                or set(channel.compatible_fidelities) & set(realization.fidelity_aliases)
            )
        )
    )
    deployments = tuple(
        ...
        if not item.compatible_fidelities
        or set(item.compatible_fidelities) & set(realization.fidelity_aliases)
    )
    spawned = tuple(
        f"{item.status}:{item.id}:{item.lifecycle}:{item.child_model_id or item.child_model_kind}"
    ...
    deployment_blockers = tuple(
        f"deployment/{item.id}: {blocker}"
        for item in deployments
        for blocker in item.blockers
    )
...
def _operation_coverage(
    ...
    exact = tuple(
        (mission_id, item)
        for mission_id, item in operations
        if getattr(item, "operation") == operation
    )
    available = tuple(
    ...
    *,
    ...
    exact = next(
        (
            ...
            if item.published_model_id == model_id and item.realization_id == realization_id
        ),
        None,
    )
    ...
    family = next(
        (
            item.status
            for item in inventory.assets
            if item.published_model_id == model_id and item.classification == "family"
        ),
        None,
    )
    if family is None:
...
def render_mission_composition_completion_markdown(
    ...
    for item in report.future_physics_backlog:
        lines.append(
            f"- **{item.id}** ({', '.join(item.families)}): {item.description} Dependency: {item.dependency}"
        )
