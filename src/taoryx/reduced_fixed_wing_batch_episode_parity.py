"""Exact action-trace parity witnesses for source-owned reduced fixed-wing steppers."""
...
from .composition_episode import (
    ...
    _f16_guidance_override,
    _f16_stepper_for_composition,
    _native_action_for_frame,
    _status_frame,
)
from .trajectory import A320RacetrackStepper, F16ReducedRacetrackStepper
...
_ADAPTER_IDS = {
    "a320_openap_3dof": "taoryx.reduced_fixed_wing.a320_action_trace_batch_episode_parity.v1",
    "f16_s119": "taoryx.reduced_fixed_wing.f16_action_trace_batch_episode_parity.v1",
}
_BATCH_FACTORIES = {
    "a320_openap_3dof": "reduced_fixed_wing_openap.v1",
    "f16_s119": "reduced_fixed_wing_f16_source.v1",
}
_EPISODE_FACTORIES = {
    "a320_openap_3dof": "reduced_fixed_wing_a320_episode.v1",
    "f16_s119": "reduced_fixed_wing_f16_episode.v1",
}
_TOLERANCE = 1.0e-12
...
class ReducedFixedWingBatchEpisodeParityReport:
    """One narrowly scoped source-owned reduced kernel/status parity report."""
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "claim_boundary": (
                "This compares one semantic kinematic-guidance trace through separately constructed source-owned "
...
def verify_serialized_reduced_fixed_wing_batch_episode_parity(
    ...
    """Replay a persisted trace through a fresh source-owned reduced batch stepper."""
    ...
    if authority_profile_id != "kinematic_guidance":
        raise ValueError("reduced fixed-wing parity requires the kinematic_guidance authority profile")
    ...
    if batch_binding.factory_id != _BATCH_FACTORIES[composition.family_id]:
        raise ValueError("reduced fixed-wing parity does not recognize the selected batch factory")
    if episode_binding.factory_id != _EPISODE_FACTORIES[composition.family_id]:
        raise ValueError("reduced fixed-wing parity does not recognize the selected episode factory")

    stepper = _stepper_for_composition(composition)
    ...
    for index, item in enumerate(_sequence(payload.get("steps"), "steps")):
        ...
        _step_stepper(stepper, composition.family_id, frame.duration_s, held_native)
        actual = _status(stepper, contract, composition.family_id)
        ...
        records.append(
            ReducedFixedWingBatchEpisodeParityStep(
                index, start_time, actual.time_s, "pass" if not mismatches else "fail", tuple(mismatches)
            )
        )
    expected_final = _mapping(payload.get("final_status"), "final_status")
    final_mismatches = _mismatches(
        _mapping(expected_final.get("values"), "final status values"), _status(stepper, contract, composition.family_id).values, "final_status"
    )
    if final_mismatches:
        now = stepper.state.time_s
        records.append(ReducedFixedWingBatchEpisodeParityStep(len(records), now, now, "fail", tuple(final_mismatches)))
    return ReducedFixedWingBatchEpisodeParityReport(
        ...
        _ADAPTER_IDS[composition.family_id],
...
def _validate_identity(composition: CompiledVehicleComposition, payload: Mapping[str, object]) -> None:
    ...
    if (
        composition.family_id not in _ADAPTER_IDS
        or composition.mission != "powered_fixed_wing_racetrack_v1"
        or composition.fidelity not in {"point_mass_3dof", "pseudo_6dof"}
    ):
        raise ValueError("no reduced fixed-wing batch/episode parity adapter is registered for this composition")
    parity = batch_episode_parity_record(composition.family_id, composition.mission, composition.fidelity)
    if parity.get("availability") != "registered" or parity.get("adapter_id") != _ADAPTER_IDS[composition.family_id]:
        raise ValueError("no reduced fixed-wing batch/episode parity adapter is registered for this composition")
...
def _stepper_for_composition(
    composition: CompiledVehicleComposition,
) -> A320RacetrackStepper | F16ReducedRacetrackStepper:
    return _a320_stepper_for_composition(composition) if composition.family_id == "a320_openap_3dof" else _f16_stepper_for_composition(composition)
    ####


def _step_stepper(
    stepper: A320RacetrackStepper | F16ReducedRacetrackStepper,
    family_id: str,
    duration_s: float,
    action: Mapping[str, float],
) -> None:
    if family_id == "a320_openap_3dof":
        if not isinstance(stepper, A320RacetrackStepper):
            raise ValueError("A320 parity adapter received the wrong stepper")
        stepper.step(duration_s, _a320_guidance_override(action))
        return
    if not isinstance(stepper, F16ReducedRacetrackStepper):
        raise ValueError("F-16 parity adapter received the wrong stepper")
    stepper.step(duration_s, _f16_guidance_override(action))
    ####
...
def _status(
    ...
    return _status_frame(contract, EpisodeObservation(stepper.state.time_s, _a320_status_values(row), lifecycle))
    ####
