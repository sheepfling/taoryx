"""Numerical-quality and fidelity-boundary gates for Product 2 artifacts.

The Product 2 run manifest answers what was executed.  This module answers
...
if TYPE_CHECKING:
    from taoryx.product_two_catalog import ProductTwoScenario
...
class ProductTwoFidelityContract(BaseModel):
...
class ProductTwoRepeatabilitySpec(BaseModel):
    ...
    def require_channels_when_enabled(self) -> ProductTwoRepeatabilitySpec:
...
class ProductTwoRefinementSpec(BaseModel):
    ...
    @model_validator(mode="after")
    def require_channels_when_enabled(self) -> ProductTwoRefinementSpec:
        if self.enabled and not self.channels:
...
class ProductTwoInvariantSpec(BaseModel):
...
class ProductTwoQualitySpec(BaseModel):
    """M4 gate declarations carried by a Product 2 catalog entry."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    fidelity: ProductTwoFidelityContract
    ...
    repeatability: ProductTwoRepeatabilitySpec = Field(default_factory=ProductTwoRepeatabilitySpec)
    refinement: ProductTwoRefinementSpec = Field(default_factory=ProductTwoRefinementSpec)
    invariants: tuple[ProductTwoInvariantSpec, ...] = ()

    @model_validator(mode="after")
    def validate_continuity_tolerances(self) -> ProductTwoQualitySpec:
...
class ProductTwoQualityCheck(BaseModel):
...
class ProductTwoNumericalQualityReport(BaseModel):
    """Complete M4 report for one canonical scenario."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    schema_id: str = Field(
        default="taoryx.product-two-numerical-quality/v1alpha1",
        alias="schema",
    ...
    claim_boundary: str = Field(min_length=1)
    checks: tuple[ProductTwoQualityCheck, ...]
...
def evaluate_product_two_quality(
    ...
    *,
...
) -> ProductTwoNumericalQualityReport:
    ...
    family-owned artifact contract but no normalized Product 2 projection.
    """

    quality = scenario.quality
    checks: list[ProductTwoQualityCheck] = []
    ...
    if primary is None:
        ...
        message = (
            ...
            else "family-owned evidence is outside the normalized Product 2 artifact gate"
        )
    ...
    return ProductTwoNumericalQualityReport(
...
def validate_product_two_quality_catalog(scenarios: Sequence[ProductTwoScenario]) -> tuple[str, ...]:
...
def _fidelity_declaration_check(scenario: ProductTwoScenario) -> ProductTwoQualityCheck:
    contract = scenario.quality.fidelity
    return _check(
...
def _claim_boundary_check(scenario: ProductTwoScenario) -> ProductTwoQualityCheck:
    contract = scenario.quality.fidelity
    return _check(
        "claim-boundary",
...
def _missing_artifact_checks(quality: ProductTwoQualitySpec, disposition: QualityDisposition) -> tuple[ProductTwoQualityCheck, ...]:
    checks: list[ProductTwoQualityCheck] = []
...
def _audit_artifact(artifact: RunArtifact, quality: ProductTwoQualitySpec) -> tuple[ProductTwoQualityCheck, ...]:
    checks: list[ProductTwoQualityCheck] = []
    if quality.finite_telemetry:
...
def _finite_telemetry_check(artifact: RunArtifact, selected_channels: Sequence[str] = ()) -> ProductTwoQualityCheck:
...
def _event_ordering_check(artifact: RunArtifact) -> ProductTwoQualityCheck:
...
def _continuity_check(artifact: RunArtifact, channels: Mapping[str, float]) -> ProductTwoQualityCheck:
...
def _invariant_check(artifact: RunArtifact, invariant: ProductTwoInvariantSpec) -> ProductTwoQualityCheck:
    reports: list[dict[str, object]] = []
    missing: list[str] = []
    for vehicle_id, vehicle in artifact.vehicles.items():
        required: tuple[str, ...]
...
def _repeatability_check(first: RunArtifact, second: RunArtifact, spec: ProductTwoRepeatabilitySpec) -> ProductTwoQualityCheck:
...
def _refinement_check(coarse: RunArtifact, fine: RunArtifact, spec: ProductTwoRefinementSpec) -> ProductTwoQualityCheck:
    errors: list[dict[str, object]] = []
    maximum_abs = 0.0
...
def _report_disposition(checks: Sequence[ProductTwoQualityCheck], expected: str) -> QualityDisposition:
...
) -> ProductTwoQualityCheck:
    return ProductTwoQualityCheck(
        id=identifier,
        disposition=disposition,
        required=required,
        message=message,
...
__all__ = [
    "InvariantKind",
    "ProductTwoFidelityContract",
    "ProductTwoInvariantSpec",
    "ProductTwoNumericalQualityReport",
    "ProductTwoQualityCheck",
    "ProductTwoQualitySpec",
    "ProductTwoRefinementSpec",
    "ProductTwoRepeatabilitySpec",
    "QualityDisposition",
    "artifact_digest",
    "evaluate_product_two_quality",
    "validate_product_two_quality_catalog",
]
####
