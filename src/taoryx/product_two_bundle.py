"""Reproducible Product 2 scenario bundles."""
...
def build_product_two_bundle(
    ...
    *,
    ...
    return_code = 0
    status = ProductTwoStatus.PASSED if run else ProductTwoStatus.INCOMPLETE
    ...
    read_product_two_run_manifest(manifest_path)
    index: dict[str, object] = {
        "schema": "taoryx.product-two-bundle/v1alpha1",
        "scenario_id": scenario.id,
        "status": status.value,
...
def _run_scenario(scenario: ProductTwoScenario, output: Path, root: Path) -> tuple[int, Path | None, dict[str, object]]:
    run_dir = output / "run"
...
def _copy_inputs(scenario: ProductTwoScenario, input_dir: Path, root: Path) -> dict[str, Path]:
...
def _copied_input_record(item: object, path: Path, output: Path) -> ProductTwoManifestSourceInput:
    catalog_input = item
    role = str(getattr(catalog_input, "role"))
    data = path.read_bytes()
    return ProductTwoManifestSourceInput(
...
def _bundle_artifacts(output: Path) -> tuple[ProductTwoManifestArtifact, ...]:
...
def _write_reproduction(output: Path, scenario: ProductTwoScenario, root: Path) -> tuple[str, ...]:
    ...
    lines = [
        "# TAORYX Product 2 reproducible bundle",
...
def _status_from_return_code(return_code: int) -> ProductTwoStatus:
    if return_code == 0:
        return ProductTwoStatus.PASSED
    if return_code == 1:
        return ProductTwoStatus.INCOMPLETE
    return ProductTwoStatus.FAILED
    ####
...
def write_composition_run_artifact(output_dir: str | Path, composition: object) -> Path | None:
    ...
    artifact = RunArtifact(
        ...
        visualization={
            "source": "ProductTwoCompositionTruthTelemetry",
            "schema_version": 1,
...
__all__ = ["build_product_two_bundle"]
####
