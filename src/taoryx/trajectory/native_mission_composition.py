...
from ..reachability_envelope import DetachedBodyTrajectory, TrajectoryResult
...
def _project_batch_result(
    ...
    if isinstance(native_trajectory, TrajectoryResult):
...
def _spawned_child_result(
    ...
    deployment = next(
        (
            ...
            if item.lifecycle == "independently_propagated" and composition.fidelity in item.compatible_fidelities
        ),
        None,
    )
...
def _sample_indices(
    ...
    if maximum_samples is not None and len(selected) > maximum_samples:
        if maximum_samples == 1:
            ...
            selected = [
                selected[round(index * (len(selected) - 1) / (maximum_samples - 1))]
...
def _applicable_bindings(model_id: str, fidelity: str, mission: str) -> tuple[NativeOutputBinding, ...]:
    return tuple(
        ...
        if (not item.fidelities or fidelity in item.fidelities)
        and (not item.mission_templates or mission in item.mission_templates)
    )
    ####
    ...
    for index, raw in enumerate(trajectory.mission_events, start=1):
        ...
        result.append(
            TrajectoryEvent(
                ...
                object_id=root_id,
                segment_instance_id=(
                    next((item.instance_id for item in composition.segments if item.id == segment), None)
...
def _resolve_mission_id(
    ...
    candidates = tuple(
        item
        for item in model.mission_templates
        if item.segment_sequence == segment_ids and fidelity in item.compatible_fidelities
    )
    if requested is not None:
        selected = next((item for item in candidates if item.id == requested), None)
        if selected is None:
            raise ValueError(
                f"mission_template_id {requested!r} does not match the configured sequence and fidelity"
            )
        return selected.id
    if len(candidates) != 1:
        raise ValueError(
            "mission_template_id is required because the configured sequence does not identify exactly one mission"
        )
