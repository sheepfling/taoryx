"""Ordered setup diagnostics for Simulation Runtime catalog scenarios."""
...
def doctor_scenario(scenario: SimulationRuntimeScenario, *, root: Path = ROOT) -> dict[str, object]:
...
def _doctor_result(
    ...
    if any(item.get("blocking") for item in diagnostics):
        status = SimulationRuntimeStatus.BLOCKED
    elif any(item.get("status") == "failed" for item in checks):
        status = SimulationRuntimeStatus.FAILED
    else:
        status = SimulationRuntimeStatus.PASSED
    return {
        "schema": "taoryx.simulation-runtime-doctor/v1alpha1",
