...
from .spectre_trajectories import SpectreTrajectoryProblemSpec, SpectreTrajectorySpec
...
class SpectrePlotSeries:
    """A synthetic 1D plot series for a Spectre trajectory artifact."""
...
def build_problem_plot_series(problem: SpectreTrajectoryProblemSpec) -> list[SpectrePlotSeries]:
    """Build a deterministic, schematic plot pack for a Spectre problem."""

    series: list[SpectrePlotSeries] = []
...
def _build_trajectory_plot_series(
    problem: SpectreTrajectoryProblemSpec,
    trajectory: SpectreTrajectorySpec,
    trajectory_index: int,
) -> list[SpectrePlotSeries]:
    ...
    plot_series: list[SpectrePlotSeries] = []
    plot_series.append(
        SpectrePlotSeries(
...
def _propnav_target_path(problem: SpectreTrajectoryProblemSpec, time_axis: tuple[float, ...]) -> list[tuple[float, float]]:
...
def _range_axis_label(_problem: SpectreTrajectoryProblemSpec, _trajectory: SpectreTrajectorySpec) -> str:
    return "range (m)"
####
...
def _classify_problem(problem: SpectreTrajectoryProblemSpec, trajectory: SpectreTrajectorySpec) -> str:
    text = " ".join(
        [
            problem.path,
...
def _trajectory_role(trajectory: SpectreTrajectorySpec) -> str:
...
def _turn_sign(problem: SpectreTrajectoryProblemSpec, trajectory: SpectreTrajectorySpec) -> float:
...
def _turn_cycles(trajectory: SpectreTrajectorySpec) -> float:
