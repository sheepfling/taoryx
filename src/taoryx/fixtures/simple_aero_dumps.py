...
class SimpleAeroSegmentSpec:
    """A rendered TAOS segment constructed from SimpleAero source metadata."""
...
@dataclass(frozen=True, slots=True)
class SimpleAeroProblemSpec:
    """A synthetic TAOS problem-file translation for one SimpleAero family."""
...
@dataclass(frozen=True, slots=True)
class SimpleAeroWorkspaceSpec:
    """A workspace-level bundle of generated SimpleAero problem files."""

    schema_version: int
    source_bundle: str
    translation_style: str
    problems: tuple[SimpleAeroProblemSpec, ...]


@dataclass(frozen=True, slots=True)
class SimpleAeroPhaseCatalogSpec:
    """A phase bucket in the higher-level SimpleAero family index."""
...
@dataclass(frozen=True, slots=True)
class SimpleAeroFamilyCatalogSpec:
    """A family entry in the higher-level SimpleAero family index."""
...
class SimpleAeroCatalogSpec:
    ...
    translation_style: str
    description: str
    phases: tuple[SimpleAeroPhaseCatalogSpec, ...]
    families: tuple[SimpleAeroFamilyCatalogSpec, ...]
...
def load_spec(path: Path) -> SimpleAeroWorkspaceSpec:
    """Load a SimpleAero dump workspace specification from YAML."""

    payload = _load_mapping(path)
    problems: list[SimpleAeroProblemSpec] = []
    ...
    ####
    return SimpleAeroWorkspaceSpec(
...
def load_catalog(path: Path) -> SimpleAeroCatalogSpec:
    """Load a higher-level SimpleAero family catalog from YAML."""

    payload = _load_mapping(path)
    phases = tuple(
        SimpleAeroPhaseCatalogSpec(
            name=str(item["name"]),
...
def group_families_by_phase(catalog: SimpleAeroCatalogSpec) -> dict[str, tuple[SimpleAeroFamilyCatalogSpec, ...]]:
    """Group cataloged families by their inferred phase bucket."""

    buckets: dict[str, list[SimpleAeroFamilyCatalogSpec]] = {}
...
def validate_catalog_against_workspace(catalog: SimpleAeroCatalogSpec, workspace: SimpleAeroWorkspaceSpec) -> None:
...
def render_problem_file(spec: SimpleAeroProblemSpec) -> str:
...
def render_workspace(spec: SimpleAeroWorkspaceSpec) -> dict[str, str]:
...
def write_workspace(spec: SimpleAeroWorkspaceSpec, destination: Path) -> None:
