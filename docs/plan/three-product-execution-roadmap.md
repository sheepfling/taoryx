# TAORYX three-product execution roadmap
...
TAORYX is not one backlog.  It is three connected products with different
definitions of done:
...
between products 2 and 3.  A registry that can describe a vehicle but cannot
turn an accepted composition into a native run is useful infrastructure, not
yet the user-facing composition product.
...
Skywalker X8, B747, A320, and F-16.  It is deliberately a product vertical
slice, not a claim that those vehicles have equal source fidelity or physical
effector maturity.

## Current baseline

### Product 1 — generic TAOS successor
...
### Product 2 — simulation, stepping, control, and artifacts
...
vehicle-specific raw state names in the language-backed path. Product 2 needs
...
## Core three-product handoff milestone

The initial cross-product handoff is complete when all of the following are
...
| Product | Delivered core boundary | Evidence retained as a release gate |
| --- | --- | --- |
