...
| Runtime-owned source adapter reuse | `source_table_fixed_wing.py` now owns the pinned X8 and B747 source-table local plants; the developer LQR scripts and Product 3 runtime registry use those same factories and operation probes | Verified for X8 and B747 local direct-wrench/surface witnesses | This removes tool/runtime construction drift, but does not create a mission translator, resource model, gain schedule, or envelope qualification. |
