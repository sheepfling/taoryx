# Mission Composition maturity plan: vehicle and mission composition

**Status:** Active Mission Composition maturity plan
...
Mission Composition is not a gallery of model names or a collection of hand-authored
...
Mission Composition is mature when a caller does not need to discover family-specific
...
> Mission Composition may simplify selection and composition, but it must never create a
...
identifier-based topology switch from the public Runtime / Composition boundary.
...
It is a discovery aid for stable Runtime / Composition hooks, not an assertion that every
...
developer evidence scripts import the same pinned builders that the Mission Composition
...
`taoryx.runtime-control-provenance/v1alpha1`.  The default Mission Composition artifact
...
The HL-20 has an explicit Mission Composition capability estimate for its public
...
execute its trim, crossrange, and energy-handoff segments. Mission Composition must not
...
These are Mission Composition gaps, not excuses to weaken the existing compiler. The
...
`qualified` is deliberately outside this ladder. Mission Composition can make a run
...
**Goal:** Make the existing Mission Composition surface easy to discover without
...
declared Mission Composition action. It does not duplicate source-intake or promotion
...
controller progress. The Mission Composition maturity report exposes verified/missing
...
| NESC staged replay and passive tumble | batch only | `not_available` | Decide whether interactive stepping is a supported Runtime capability; passive/uncontrolled paths may legitimately remain batch-only. |
...
between a stable Mission Composition API and source-owned runtime coordinates:
...
intake compiler is now also a Mission Composition command:
...
The source-family work is now discoverable through the same Mission Composition CLI,
...
`taoryx vehicle maturity-report` embeds the same report, so Mission Composition cannot
...
- no new physical family enters the catalog without a declared Mission Composition
  composition contract.

## Priority and Alpha placement

| Horizon | Mission Composition work | Why it comes first |
| --- | --- | --- |
| Current / Alpha 2 closeout | M0, plus preserve existing compose-to-run witnesses. | Makes the existing Mission Composition layer discoverable and honest. |
...
| Alpha 3 continuation | M4 and initial M5. | Makes Mission Composition useful to AI/RL, batch search, and showcase generation. |
...
terminal-handoff evidence. The immediate Mission Composition frontier is no longer a
...
data, or evidence; Mission Composition must not fill those gaps with a nearby vehicle,
...
resulting coupled state. The Mission Composition maturity target is broader *source-
...
## Mission Composition next-phase work packages
...
| `P3-AUTH-01` | Mission Composition authoring/catalog | Give users one discoverable path from vehicle metadata and bounded parameters to a compiled trajectory. | `P3-COMP-01`, `P3-COMP-02`, `P3-EXEC-01` | Catalog/describe/schema/interface/endpoints projections; authoring kit; parameter and value-space topology; compose/lower/materialize/run/preflight surfaces; blocker report. | A user can query a vehicle, select valid parameters and segments, compile a trajectory without editing code, and receive either an executable binding or a precise blocker. |
...
5. **R4 — Mission Composition maturity gate:** complete `P3-EXEC-01` and `P3-AUTH-01`,
...
manifest, and documentation projection is incomplete Mission Composition work.

## Mission Composition success measures
...
- Promoting a direct-wrench bridge, pseudo-6DOF response, or nominal mission
  solely because Mission Composition can construct and execute it.

Mission Composition is successful when it makes the right path easy: select declared
