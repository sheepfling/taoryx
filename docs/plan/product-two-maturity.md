...
The maturity objective is to make Product 2 a predictable, self-describing
simulation product. The next level is not simply more vehicle names; it is a
...
- freeze the Product 2 status vocabulary: `passed`, `development`, `blocked`,
  `incomplete`, `out_of_envelope`, and `failed`;
- define the minimum run-manifest fields and schema compatibility policy in
  [the Product 2 run-manifest contract](../architecture/product-two-run-manifest.md); and
- record baseline timings, artifact sizes, and deterministic repeatability for
  the canonical regression set in
  [`verification/product_two_baseline.json`](../../verification/product_two_baseline.json),
  generated by [`tools/build_product_two_baseline.py`](../../tools/build_product_two_baseline.py).
...
Purpose: make Product 2 maintainable as a product rather than a collection of
working examples.
...
| G1 | Is the product discoverable? | Catalog plus `scenario show`/`doctor` for every canonical case. |
