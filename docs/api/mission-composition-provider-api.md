...
The canonical TAORYX-native Python surface is
[`taoryx.trajectory.mission_composition`](../../src/taoryx/trajectory/mission_composition.py).
The independent public composition/batch/streaming interface is the separately
installable `taoryx-trajectory-contracts` package; TAORYX implements it through
`TaoryxTrajectoryContractsAdapter`. See
[Standalone trajectory contracts](trajectory-contracts.md) for the package
boundary, external-provider rules, and consumer flow.
For the role-first split between that external seam and TAORYX's internal
vehicle/model plug-in path, see [Developer interface
...
value space. See [Control semantics and agent action spaces](../architecture/control-agent-action-space.md)
...
the checked-in
[family/realization coverage matrix](../architecture/mission-composition-coverage-matrix.md).
