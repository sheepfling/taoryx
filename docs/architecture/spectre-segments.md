...
The Spectre material is valuable as a compact library of maneuver shapes, but
the checked-in problem files are synthetic translations and not a recovered
Spectre runtime. The reusable unit is therefore the segment contract, not the
literal `.prb` text.
...
| Contract | Typical Spectre families | Reusable intent |
| --- | --- | --- |
...
`taoryx.spectre_builder`. The small input surface is intentionally shaped like
...
vehicle model or claim to reproduce every family-specific Spectre law.
...
## Validate the Spectre segment fixtures

Run the isolated Spectre ladder with:
...
controls, and valid operating envelope. Do not copy a Spectre bank value or
...
## Spectre catalog and evidence ladder

`verification/spectre_segment_catalog.yaml` maps each fixture family to its
