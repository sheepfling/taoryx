# Simulation Runtime numerical quality and fidelity gates

Status: M4 implementation contract

Simulation Runtime separates a runnable trajectory from a numerically bounded result.
...
Every entry in `verification/simulation_runtime_scenario_catalog.yaml` carries a
...
`taoryx.simulation_runtime_quality.evaluate_simulation_runtime_quality` emits
`taoryx.simulation-runtime-numerical-quality/v1alpha1` for one scenario. The report
...
```bash
python tools/validate_simulation_runtime_quality.py --output artifacts/verification/simulation_runtime_quality/report.json
```
...
```bash
python tools/validate_simulation_runtime_quality.py \
  --execute \
...
common Simulation Runtime artifact adapter is declared.
