# Simulation Runtime: find, set up, step, and diagnose a scenario

This is the shortest route for a new engineer or a consumer who wants to run
an existing TAORYX model. Simulation Runtime is the simulation and stepping platform:
...
| Run artifact | Normalized Simulation Runtime telemetry and event data from a source run or step session. | caller-selected `artifacts/` directory |
...
For the canonical Simulation Runtime set, start with the machine-readable discovery
...
[Simulation Runtime discovery and bundles](architecture/simulation-runtime-discovery-and-bundles.md).
The numerical-quality and fidelity declaration contract is in
[Simulation Runtime quality gates](architecture/simulation-runtime-quality-gates.md).
...
taoryx run \
  ...
  --integrator rk4 \
  --max-steps 20000 \
  --output-dir artifacts/simulation-runtime/two-stage-ballistic \
  --report artifacts/simulation-runtime/two-stage-ballistic/run-report.json \
  --artifact artifacts/simulation-runtime/two-stage-ballistic/run-artifact.json

taoryx artifact inspect \
  artifacts/simulation-runtime/two-stage-ballistic/run-artifact.json

taoryx artifact plot \
  artifacts/simulation-runtime/two-stage-ballistic/run-artifact.json \
  --output-dir artifacts/simulation-runtime/two-stage-ballistic/plots
```
...
`RunArtifact` is the stable Simulation Runtime telemetry surface used by plots and
...
taoryx scenario compile \
  ...
  examples/vehicle_families/staged_rocket/two_stage_demo/propulsion.tbl \
  --profile taoryx \
  --output artifacts/simulation-runtime/two-stage-demo/scenario.json \
  --json
```
...
taoryx vehicle compose \
  ...
  --output artifacts/simulation-runtime/nesc-pseudo6dof/composition.json

taoryx vehicle preflight \
  artifacts/simulation-runtime/nesc-pseudo6dof/composition.json
...
taoryx vehicle run \
  artifacts/simulation-runtime/nesc-pseudo6dof/composition.json \
  --output-dir artifacts/simulation-runtime/nesc-pseudo6dof/run

taoryx vehicle result \
  artifacts/simulation-runtime/nesc-pseudo6dof/run
```
...
taoryx vehicle compose \
  ...
  --output artifacts/simulation-runtime/x15-staged-pseudo6dof/composition.json

taoryx vehicle run \
  artifacts/simulation-runtime/x15-staged-pseudo6dof/composition.json \
  --output-dir artifacts/simulation-runtime/x15-staged-pseudo6dof/run

taoryx vehicle result \
  artifacts/simulation-runtime/x15-staged-pseudo6dof/run
```
...
taoryx vehicle compose \
  ...
  --output artifacts/simulation-runtime/hl20-source-release-pseudo6dof/composition.json

taoryx vehicle run \
  artifacts/simulation-runtime/hl20-source-release-pseudo6dof/composition.json \
  --output-dir artifacts/simulation-runtime/hl20-source-release-pseudo6dof/run

taoryx vehicle result \
  artifacts/simulation-runtime/hl20-source-release-pseudo6dof/run
```
...
python tools/validate_hl20_terminal_contract.py \
  --output artifacts/simulation-runtime/hl20-ca-hi/terminal-contract.json
```
...
1. **Synthetic CA–HI route** — run
   ...
   --output-dir artifacts/simulation-runtime/california-to-hawaii`. This is a native
...
Simulation Runtime has one runtime behind both batch and interactive execution. A
...
session.to_run_artifact().write_json(
    "artifacts/simulation-runtime/california-to-hawaii/interactive-artifact.json"
)
```
...
- `evaluation.json` — normalized Mission Composition projection;
...
```bash
python tools/validate_simulation_runtime_quality.py --execute \
  --scenario two-stage-ballistic \
  --scenario two-stage-demo \
  --scenario interactive-california-hawaii \
  --output artifacts/verification/simulation_runtime_quality/report.json
```
...
## Simulation Runtime maturity boundary

The current Simulation Runtime baseline is strong enough for deterministic source batch
